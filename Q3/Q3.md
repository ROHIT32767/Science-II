...
`$ python3 Q3.py`
```cpp
// code for eigen value plot for D = 0,1,5,10
import numpy as np
...
b) As the value of D increases , the eigen values will move away from the origin of complex plane (-D is used in the matrix)
```
```
...
```
```cpp
// ! Here the matrices are obtained after fetching a skew-symmetric matrix and then replacing the Diagonal matrix with -D
```
